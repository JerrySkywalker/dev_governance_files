...
Assert ($config.global_invariants.authority_never_expands_automatically) 'Automatic authority expansion must be false'
Assert ($config.global_invariants.budget_domains_never_borrow) 'Budget borrowing must be false'
...
foreach ($profileProperty in $config.profiles.PSObject.Properties) {
    ...
    for ($i = 0; $i -lt $domains.Count; $i++) {
        $b = @($profile.budget_matrix[$i])
        Assert ($b.Count -eq 3) "$($profileProperty.Name)/$($domains[$i]) budget tuple invalid"
        Assert ($b[2] -ge $b[0]) "$($profileProperty.Name)/$($domains[$i]) lifetime below window"
    }
}
...
$tx = $config.profiles.PROTECTED_TRANSACTION_V2
Assert (@($tx.budget_matrix[$domainIndex['apply']])[0] -eq 1) 'Apply window is frozen at 1'
Assert (@($tx.budget_matrix[$domainIndex['finalize']])[0] -eq 1) 'Finalize window is frozen at 1'
Assert ($tx.transaction_invariants.failed_or_unverified_rollback_hard_stop) 'Rollback hard stop required'
